...
export const Parameters = () => {
  ...
  const [form, setForm] = useState({ name:'', unit:'', specification:'', display_order:'', active:true });
  const [editing, setEditing] = useState(null);

  useEffect(()=>{ api.get('/report-types').then(r=>setTypes(r.data)).catch(()=>{}); }, []);
  const fetch = async () => {
    if (!selected) { setList([]); return; }
    const r = await api.get('/parameters', { params:{ report_type_id:selected }});
    setList(r.data);
  };
  ...
  const submit = async (e) => {
    e.preventDefault();
    try {
      const payload = { ...form, report_type_id: selected, display_order: form.display_order ? parseInt(form.display_order) : undefined };
      ...
      setForm({ name:'', unit:'', specification:'', display_order:'', active:true }); setEditing(null); fetch();
    } catch(err){ addToast(err.response?.data?.message||'Failed','error'); }
  };

  const toggle = async (p) => {
    try { await api.put(`/parameters/${p.id}`, { active: !p.active }); fetch(); } catch {}
  };
        ...
        <>
          <form onSubmit={submit} className="bg-white border border-[#D1D5DB] rounded-2xl p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="sm:col-span-2 flex gap-2">
              <button type="submit" className="px-4 py-2 rounded-xl bg-[#168B57] text-white font-bold text-xs">{editing?'Update':'Add Parameter'}</button>
              {editing && <button type="button" onClick={()=>{setEditing(null); setForm({name:'',unit:'',specification:'',display_order:'',active:true});}} className="px-3 py-2 rounded-xl border border-[#D1D5DB] text-xs">Cancel</button>}
            </div>
          </form>
                ...
                {list.map(p=>(
                  <tr key={p.id}>
                    ...
                    <td className="py-2 px-3 text-right flex items-center justify-end gap-1">
                      <button onClick={()=>{setEditing(p.id); setForm({name:p.name, unit:p.unit||'', specification:p.specification||'', display_order:p.display_order, active:p.active});}} className="px-2 py-1 rounded-lg border border-[#D1D5DB] text-xs">Edit</button>
